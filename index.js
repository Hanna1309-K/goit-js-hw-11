import{a as d,S as u,i as l}from"./assets/vendor-CNqCr-V-.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))o(e);new MutationObserver(e=>{for(const s of e)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function t(e){const s={};return e.integrity&&(s.integrity=e.integrity),e.referrerPolicy&&(s.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?s.credentials="include":e.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(e){if(e.ep)return;e.ep=!0;const s=t(e);fetch(e.href,s)}})();const f="54320927-5ca4d41cc3ecb718a2e32b522",m="https://pixabay.com/api/";async function h(r){return(await d.get(m,{params:{key:f,q:r,image_type:"photo",orientation:"horizontal",safesearch:!0}})).data}const p=document.querySelector(".gallery"),n=document.querySelector(".loader"),C=new u(".gallery a",{captionsData:"alt",captionDelay:250});function g(r){const a=imagesg.map(t=>`
      ...
      `).join("");p.innerHTML=a,C.refresh()}function L(){p.innerHTML=""}function y(){n.classList.remove("hidden"),n.textContent="Loading images, please wait..."}function w(){n.classList.add("hidden")}const v=document.querySelector(".form");v.addEventListener("submit",async r=>{r.preventDefault();const a=r.target.elements["search-text"].value.trim();if(!a){l.warning({message:"Please enter a search query!",position:"topRight"});return}L(),y(),await new Promise(t=>setTimeout(t,50));try{const t=await h(a);if(t.hits.length===0){c();return}g(t.hits)}catch{c()}finally{w()}});function c(){l.error({position:"topRight",backgroundColor:"#EF4040",timeout:3e3,close:!1,icon:"",message:`
